...
/**
 * Computes a new importance value when a task is drag-reordered within a list
 * sorted by importance **descending** (index 0 = highest importance).
 *
 * Algorithm: after the virtual move, place the task between its new neighbours:
 * - Both neighbours present: midpoint. If they are adjacent integers (gap = 1),
 *   return the higher value (ties are acceptable).
 * - First position (no higher neighbour): proportional step above the lower
 *   neighbour so the result is strictly greater.
 * - Last position (no lower neighbour): one below the higher neighbour.
 * - Single task (no neighbours): unchanged.
 *
 * @param bucketBounds - Optional min/max importance bounds for the current
 *   swimlane or column bucket. When provided the result is clamped to keep
 *   the task within the same importance bucket after the drop.
 */
export function computeImportanceForReorder(
	tasks: Task[],
	fromIndex: number,
	toIndex: number,
	bucketBounds?: { min: number; max: number },
): number {
	if (fromIndex === toIndex || tasks.length === 0) {
		return tasks[fromIndex]?.importance ?? 0;
	}
	...
	const higher = reordered[toIndex - 1]; // neighbour above → higher importance
	const lower = reordered[toIndex + 1]; // neighbour below → lower importance

	let result: number;

	if (higher !== undefined && lower !== undefined) {
		const hi = higher.importance;
		const lo = lower.importance;
		if (hi <= lo) {
			// Data is not cleanly ordered or tied — keep at the higher value
			result = hi;
		} else {
			// Normal midpoint; for a gap of 1 this rounds to hi (acceptable tie)
			result = Math.round((hi + lo) / 2);
		}
	} else if (higher === undefined && lower !== undefined) {
		// First position: must be strictly above the lowest visible neighbour.
		// Use a proportional step so the result stays within the same bucket range.
		const step = Math.max(1, Math.ceil(lower.importance / 2));
		result = lower.importance + step;
	} else if (lower === undefined && higher !== undefined) {
		// Last position: one step below the highest visible neighbour.
		result = Math.max(0, higher.importance - 1);
	} else {
		// Single task — no meaningful position change.
		return moved.importance;
	}

	// Clamp to bucket bounds when operating within a swimlane/column bucket.
	if (bucketBounds !== undefined) {
		result = Math.max(bucketBounds.min, Math.min(bucketBounds.max, result));
	}
